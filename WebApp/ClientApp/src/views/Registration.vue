        ...
        <v-container>
          <v-layout wrap>
            ...
            <v-flex xs6>
              <v-text-field
                class="form-control"
                :class="{ 'is-invalid': submitted && !group }"
                v-model="group"
                prepend-icon="group"
            ...
            <v-flex xs6>
              <v-text-field
                class="form-control"
                :class="{ 'is-invalid': submitted && !studentCode }"
                v-model="studentCode"
            ...
            <v-flex xs11>
              <v-text-field
                class="form-control"
                :class="{ 'is-invalid': submitted && !uid }"
                v-model="uid"
            ...
            <v-flex xs1>
            <v-btn flat icon color="green"  @click="updateUid">
            ...
            <v-flex xs12>
              <v-text-field
                class="form-control"
                :class="{ 'is-invalid': submitted && !verificationPassword }"
...
export default {
  ...
  computed: {
    ...
    passwordErrors() {
      ...
      !this.$v.password.minLength &&
        errors.push("Slaptažodis turi būti iš bent 8 simbolių");
      !(this.password === this.verificationPassword) &&
        errors.push("Neteisingai pakartojote slaptažodį");
    ...
    verificationPasswordErrors() {
      ...
      !this.$v.verificationPassword.minLength &&
        errors.push("Slaptažodis turi būti iš bent 8 simbolių");
      !(this.password === this.verificationPassword) &&
        errors.push("Neteisingai pakartojote slaptažodį");
      !this.$v.verificationPassword.required &&
        errors.push("Privalomas laukas");
      return errors;
    },
    emailErrors() {
      const errors = [];
  ...
  methods: {
    updateUid() {
      ...
      axios
        .get("api/nfcscan/getlastscan",{ headers: headers})
        .then(response => {
          //this.uid = response["uid"];
          this.uid = response["data"]["scan"]["uid"]
